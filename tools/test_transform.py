...
if __name__ == "__main__":
    ori_data = np.array([
        [535.0, 269.0, 518.0, 301.0, 459.0, 268.0, 478.0, 236.0],
        [1414.0, 784.0, 1239.0, 1034.0, 206.0, 250.0, 408.0, 17.0],
    ], dtype=np.float32)   
    rbox_le90 = [[poly2obb_np_le90(ord)] for ord in ori_data]
    rbox_le90 = np.array(rbox_le90)
    print("11111111",rbox_le90)
    rbox_oc = [[poly2obb_np_oc(ord)] for ord in ori_data]
    rbox_oc = np.array(rbox_oc)
    print("2222222",rbox_oc)
    new_gt_bboxes_list = []
    for rbox in rbox_le90:
        # 将每个batch中的5D框转换为8D多边形
        poly_bboxes = obb2poly_np_le90(rbox)
        # print("#########poly_bboxes:",poly_bboxes)
        poly_bboxes_f32 = poly_bboxes.astype(np.float32) 
        obb_bboxes_oc = poly2obb_np_oc(poly_bboxes_f32)
        new_gt_bboxes_list.append(obb_bboxes_oc)
        print('&&&&&&new_gt_bboxes_list',new_gt_bboxes_list)
