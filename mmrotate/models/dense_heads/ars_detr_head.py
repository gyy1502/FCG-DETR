...
class ARSDeformableDETRHead(RotatedDETRHead):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        assert angle_coder['angle_version'] == 'le90', \
            'should use le90 angle_version'
        self.aspect_ratio_weighting = aspect_ratio_weighting
        ...
        super(ARSDeformableDETRHead, self).__init__(
            *args, transformer=transformer, **kwargs)
        self.loss_angle = build_loss(loss_angle)

    def _init_layers(self):  ###原本一层的
        """Initialize classification branch and regression branch of head."""

        fc_cls = Linear(self.embed_dims, self.cls_out_channels)
        fc_angle_cls = Linear(self.embed_dims, self.coding_len)
        reg_branch = []
        ...
        if self.with_box_refine: #####执行
            self.cls_branches = _get_clones(fc_cls, num_pred)
            self.reg_branches = _get_clones(reg_branch, num_pred)
            self.angle_branches = _get_clones(fc_angle_cls, num_pred)
        else:
            ...
            self.angle_branches = nn.ModuleList(
                [fc_angle_cls for _ in range(num_pred)])

        self.angle_coders = [self.angle_coder for _ in range(num_pred)]
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        if self.as_two_stage:
            return outputs_classes, outputs_coords, outputs_anlges, outputs_angle_clses,\
                enc_outputs_class, \
                enc_outputs_coord.sigmoid(), enc_outputs_angle_cls
        else:
            return outputs_classes, outputs_coords, outputs_anlges, outputs_angle_clses, \
                None, None, None
    ...
    def get_bboxes(self,
        ...
        angle_preds = all_angle_preds[-1]

        result_list = []
        for img_id in range(len(img_metas)):
            cls_score = cls_scores[img_id]
            bbox_pred = bbox_preds[img_id]
            angle_pred = angle_preds[img_id]
            img_shape = img_metas[img_id]['img_shape']
            scale_factor = img_metas[img_id]['scale_factor']
            proposals = self._get_bboxes_single(cls_score, bbox_pred, angle_pred,
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            angle = angle_pred[bbox_index].view(-1, 1)
            bbox_pred = torch.cat((bbox_pred, angle), dim=1)
            # pass
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        outs = self.forward(feats, img_metas)[:7]  ##截断
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         angle_list, angle_weights_list, pos_inds_list, neg_inds_list) = multi_apply(
            self._get_target_single, cls_scores_list, bbox_preds_list, angle_preds_list, angle_cls_list, angle_coder,
            gt_bboxes_list, gt_labels_list, img_metas, gt_bboxes_ignore_list)
        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))
        return (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
                angle_list, angle_weights_list, num_total_pos, num_total_neg)
