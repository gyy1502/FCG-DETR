...
from mmrotate.models.detectors.fcg_criterion import HFGCriterion
...
class DNARSDeformableDETRHead(ARSDeformableDETRHead):
    ...
    def __init__(self,
        ...
        self.focus_criterion_dict = kwargs.get('focus_criterion',None)
    ...
    def forward_train(self,
        ...
        self.focus_criterion = HFGCriterion(noise_scale=self.focus_criterion_dict['noise_scale'],\
                                                alpha=self.focus_criterion_dict['alpha'], \
                                                gamma=self.focus_criterion_dict['gamma'],\
                                                weight = self.focus_criterion_dict['weight'],\
                                                save_gauss_results=self.focus_criterion_dict['save_gauss_results']) 
        outs = self(x, img_metas, dn_label_query, dn_bbox_query, attn_mask, gt_bboxes, gt_labels) 
    ...
    def forward(self,
        ...
        # compute focus loss
        feature_stride = [(
           input_img_h / feature.shape[-2],
           input_img_w / feature.shape[-1],
        ) for feature in mlvl_feats]   

        image_sizes = [torch.Size(meta['batch_input_shape']) for meta in img_metas]

       # 假设 gt_bboxes 和 gt_labels 是以列表形式存在的，并且每个列表包含多个元素
        ...
        enc_outputs_class, enc_outputs_coord, enc_outputs_angle_cls,salience_score = self.transformer(
            mlvl_feats,
            mlvl_masks,
            query_embeds,
            mlvl_positional_encodings,
        ...
        hs = hs.permute(0, 2, 1, 3)  #[6,2,498,256]
        ...
        return outputs_classes, outputs_coords, outputs_anlges, outputs_angle_clses,\
            enc_outputs_class, enc_outputs_coord.sigmoid(), enc_outputs_angle_cls,salience_score,feature_stride,image_sizes,mlvl_feats ##增加几个返回变量
    ...
    def loss(self,
             ...
             dn_meta=None,
             gt_bboxes_ignore=None):
        #outputs_classes, outputs_coords, outputs_anlges, outputs_angle_clses,
        #enc_outputs_class, enc_outputs_coord.sigmoid(), enc_outputs_angle_cls,
        # salience_score,feature_stride,image_sizes,mlvl_feats，gt_bboxes, gt_labels,
        #  img_metas, dn_meta
        ...
        if img_metas is not None:
            # 使用列表推导式提取所有 filename
            filenames = [img_meta['filename'] for img_meta in img_metas]
        ...
        if salience_score is not None:
            ...
            loss_focus = self.focus_criterion(salience_score, gt_bboxes_list,gt_labels_list, feature_strides,image_sizes,filenames)
            # loss_dict[f'loss_focus'] = loss_focus
            for nn,loss_focus_i in enumerate(loss_focus): 
                assert isinstance(loss_focus_i, torch.Tensor), f"Expected tensor, got {type(loss_focus_i)}"                                               
                loss_dict[f'f{nn}.loss_focus'] = loss_focus_i
        return loss_dict

     # --- START: NEW METHOD ---
    def loss_focus(self, feat, salience_score_level, gt_bboxes_list, 
                          gt_labels_list, stride, image_sizes, filenames):
        """
        Compute focus loss for a single feature level.
        This function is designed to be called by `multi_apply`.
        """
        # The `focus_criterion` expects lists of features and strides.
        # We wrap the single-level inputs in lists to match its API.
        # It also returns a list of losses. Since we pass one level,
        # we expect a list with one loss tensor, which we extract.
        loss_list = self.focus_criterion(
            [feat],                     # Feature map for this level, wrapped in a list
            salience_score_level,       # Salience score for this level (assuming criterion handles split scores)
            gt_bboxes_list,
            gt_labels_list,
            [stride],                   # Stride for this level, wrapped in a list
            image_sizes,
            filenames
        )
        # Assuming focus_criterion correctly returns a list with one item for a single-level input
        return loss_list[0]
