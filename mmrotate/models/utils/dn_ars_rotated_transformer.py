...
class DNARSRotatedDeformableDetrTransformer(Transformer):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(DNARSRotatedDeformableDetrTransformer, self).__init__(**kwargs)
    ...
    def forward(self,
        ...
        #     (1, )), spatial_shapes.prod(1).cumsum(0)[:-1])) ##原版本会报错，已经替换掉
        spatial_shapes_area = spatial_shapes[:, 0] * spatial_shapes[:, 1]
        level_idx = spatial_shapes_area.cumsum(0)[:-1]
        level_start_index = torch.cat((spatial_shapes.new_zeros(
            (1, )), level_idx))       
